...
class MainAppPage(tk.Frame):
    def __init__(self, parent, controller, golfkeypoints_client, session_token):
        ...
        self.open_button = tk.Button(self, text="Select New Input Image", command=self.open_new_image)
        self.open_button.grid(row=1, column=0, padx=5, pady=5, sticky="w")

        self.open_button = tk.Button(self, text="Show Previous Input Images", command=self.list_images)
        self.open_button.grid(row=4, column=0, padx=5, pady=5, sticky="w")

        #self.open_button = tk.Button(self, text="Calibrate Image", command=self.open_image)
        #self.open_button.grid(row=2, column=0, padx=5, pady=5, sticky="w")

        #self.open_button = tk.Button(self, text="Calculate Golf Keypoints", command=self.open_image)
    ...
    def list_images(self):
        ...
        try:
            messagebox.showinfo("Blah", f"Session token is {self.session_token}")
            response = self.golfkeypoints_client.list_input_images_for_user(session_token=self.session_token)
            messagebox.showinfo("Blah", f"response is {response}")
            for i, input_image_id in enumerate(response.input_image_ids):
                curr_button = tk.Button(self.canvas, text=f"{input_image_id}", command=partial(self.display_image, input_image_id))
                #curr_button.grid(row=i, column=0, padx=5, pady=5)
                self.canvas.create_window(0, i*10, window=curr_button)
        except grpc.RpcError as e:
            messagebox.showerror("List Images Failed", f"Could not get a list of images: {e.code()}: {e.details()}")
    
    def display_image(self, input_image_id):
        self.canvas.delete("all")
        try:
            ...
            #self.img = Image.frombytes(self.img.mode, self.img.size, response.image)
            buffer = BytesIO(response.image)
            self.img = Image.open(buffer)
            resized_img = self.img.resize((270, 600), Image.Resampling.LANCZOS)
