...
func testMainCodeFlowDtl(ctx context.Context, uClient skp.UserServiceClient, gClient skp.GolfKeypointsServiceClient) {
	...
	newBodyDatapoints := &skp.Body25PoseDatapoints{
	...
	updateBodyDatapointsResponse, err := gclient.UpdateBodyDatapoints(ctx, gClient, registerUserResponse.SessionToken, uploadInputImageResponse.InputImageId, newBodyDatapoints)
	if err != nil {
		log.Fatalf("Failed to update body datapoints: %s", err.Error())
	}
	log.Printf("Update body datapoints dtl: %+v", updateBodyDatapointsResponse.UpdatedGolfKeypoints.DtlGolfSetupPoints)
}
